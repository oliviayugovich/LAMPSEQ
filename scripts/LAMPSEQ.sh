...
echo "Usage: 
  ...
  Note: Script to run LAMP Sequencing analysis pipeline"
exit 2
}
...
echo "Setting up LAMPSEQ workflow for analysis"
...
echo "LAMPSEQ workflow set-up completed successfully"

# 02 - Quality Control
  # FASTQC each barcoode*.fastq file separately
...
for file in ${OUTPUT_DIR}/*.fasta; do
    ...
    echo -ne \\r"Aligning ${BASENAME} to LAMPSEQ species"
    if [[ "$DEBUG" = true ]]; then
        echo ""
    fi 
...
echo "LAMPSEQ analysis pipeline completed successfully - ready for R analysis"
...
Rscript -e "rmarkdown::render(
  input = '${SCRIPT_DIR}/species_id/LAMPSEQ_run_summary.Rmd', 
  params = list(
  indir = '${INDIR}', 
  results_dir = '${RESULTS_DIR}',
  fonts_dir = '${FONTS_DIR}'),
  output_file = paste0('LAMPSEQ_', '${INDIR_NAME}', '_run_summary.pdf'),
